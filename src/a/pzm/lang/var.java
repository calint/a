...
//		mark_end_of_source(r);
	...
	@Override public void binary_to(xbin x){
		if(x.is_alias_declared(token)){
			throw new compiler_error(this,"var '"+token+"' already used",x.register_aliases.toString());
		}
//		if(!x.is_register_alias_exists(var_name)){
			final String reg=x.allocate_register(this);
			x.alias_register(token,reg);
...
//		}
		if(initial_value_expression!=null){
			if(x.is_alias_declared(initial_value_expression.token)){
				...
				final int rdi=x.register_index_for_alias(this,token);
